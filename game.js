...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (wi && cc) {
        wi.addEventListener('input', function() {
            ...
            if (wordsCount > 120) {
                // Strictly enforce 120 words max by matching up to 120 chunks of non-whitespace
...
function createLiveRoom() {
    ...
    const shortId = "INSG-" + Math.random().toString(36).substring(2,6).toUpperCase();
...
function broadcastStartRound() {
    ...
    // Filter out prompts that have already run this match
...
function renderRevealStage() {
    ...
    // Check if the current pool is out of unique questions
    const pool = QUESTIONS[room.currentCategory] || QUESTIONS.spicy;
